...
class DAOCliente
{
    ...
    function buscarCliente($id)
    {
         ...
         $sql="SELECT * FROM cliente2 WHERE id_cliente2='$id'";
        ...
        //http://localhost:8888/EjerciciosParaEntrega/Prueba4.5/Controlador/controladorBuscarUsuario.php?idUsuario=algo%27+OR+1%3D1&buscarU=buscarU --> no dio nada
        
        //http://localhost:8888/EjerciciosParaEntrega/Prueba4.5/Controlador/controladorBuscarUsuario.php?idUsuario=2&buscarU=buscarU
        
        //http://localhost:8888/EjerciciosParaEntrega/Prueba4.5/Controlador/controladorBuscarUsuario.php?idUsuario=algoOR 1=1&buscarU=buscarU
    ...
    function borrarCliente($id){
        ...
        if ($resulatdos->num_rows > 0) {
            ...
            $sql5="SELECT * FROM datos_amort WHERE id_cliente2='$id'";
            $res=$this->conexion->conexion->query($sql5);
            if($res->num_rows > 0) {
                $ides=$res->fetch_array(MYSQLI_ASSOC);

                $sql4 = "delete from amoriacion where id_datos_amort = ?";
                $stmt = $this->conexion->conexion->prepare($sql4);
                $stmt->bind_param("i", $ides['id_datos_amort']);
                $stmt->execute();

                $sql3 = "delete from datos_amort where id_cliente2 = ?";
                $stmt = $this->conexion->conexion->prepare($sql3);
                $stmt->bind_param("i", $id);
                $stmt->execute();

                $sql2 = "delete from cliente2 where id_cliente2 = ?";
                $stmt = $this->conexion->conexion->prepare($sql2);
                $stmt->bind_param("i", $id);
                $stmt->execute();
            ...
            return "No hay usuario con ese id";
        }
...
/*
    function actualizarCliente($id){
        $nom="a";
        $email="as@fed";
        $sql="UPDATE cliente SET nombre=$nom, email=$email WHERE id_cliente=$id";

    }*/
//$sql = "UPDATE `apidis`.`cliente` SET `nombre` = \'lopez\', `email` = \'kdk@hos\' WHERE `cliente`.`id_cliente` = 3;";
